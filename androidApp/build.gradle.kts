...
android {
    ...
    defaultConfig {
        applicationId = "com.saferill.sonara"
        minSdk = 26
        targetSdk = 36
        versionCode =
    ...
    val releaseKeystore = rootProject.file(".release-signing/sonara-release.jks")
    val keystorePassword = System.getenv("KEYSTORE_PASSWORD")
        ?: localProps.getProperty("KEYSTORE_PASSWORD")
        ?: project.findProperty("KEYSTORE_PASSWORD")?.toString()
        ?: if (releaseKeystore.exists()) "SonaraMusic2026!" else ""
    ...
    signingConfigs {
        if (releaseKeystore.exists() && keystorePassword.isNotEmpty()) {
            create("release") {
    ...
    buildTypes {
        release {
            ...
            if (releaseKeystore.exists() && signingConfigs.findByName("release") != null) {
