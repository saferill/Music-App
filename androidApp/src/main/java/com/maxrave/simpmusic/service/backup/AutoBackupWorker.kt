...
class AutoBackupWorker(
    ...
    private fun saveToDownloads(backupFile: File): Boolean {
        ...
        val fileName = "sonara_music_backup_$timestamp.zip"
        ...
        return try {
            val contentValues = ContentValues().apply {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    put(MediaStore.Downloads.RELATIVE_PATH, "Download/Sonara Music")
                }
            }
            ...
            uri?.let { outputUri ->
                ...
                Logger.i(TAG, "Backup saved to Downloads/Sonara Music/$fileName")
                true
            } ?: false
        } catch (e: Exception) {
    ...
    private fun cleanupOldBackups(maxFiles: Int) {
        try {
            ...
            val selectionArgs = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                arrayOf("Download/Sonara Music/", "sonara_music_backup_%.zip")
            } else {
                arrayOf("sonara_music_backup_%.zip")
            }
            ...
            context.contentResolver.query(
                ...
                while (cursor.moveToNext()) {
                    ...
                    if (name.startsWith("sonara_music_backup_") && name.endsWith(".zip")) {
