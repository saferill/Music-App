...
object CrashDialog {
    ...
    private fun showCrashDialog(thread: Thread, throwable: Throwable) {
        ...
        val versionInfo = try {
            "Sonara Music Desktop v${VersionManager.getVersionName()}"
        } catch (_: Exception) {
            "Sonara Music Desktop"
        }

        val dialog = JDialog().apply {
            title = "Sonara Music - Unexpected Error"
            isModal = true
        ...
        val headerPanel = JPanel(BorderLayout(8, 4)).apply {
            val titleLabel = JLabel("Sonara Music has crashed").apply {
