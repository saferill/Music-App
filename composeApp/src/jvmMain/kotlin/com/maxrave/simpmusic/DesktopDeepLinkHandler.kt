 ...
 * - sonara://open-app?url=<encoded_url>  (redirected from website)
 * - sonara://watch?v=VIDEO_ID            (direct scheme)
 * - sonara://playlist?list=PLAYLIST_ID   (direct scheme)
 * - sonara://channel/CHANNEL_ID          (direct scheme)
 * - sonara://album?id=ALBUM_ID           (direct scheme)
 * - https://musicapp-lime.vercel.app/app/... (web URL passed via args)
 */
object DesktopDeepLinkHandler {
    private const val TAG = "DesktopDeepLinkHandler"

    private val pendingUriFile: File by lazy {
        File(System.getProperty("java.io.tmpdir"), "sonara_pending_deeplink.txt")
    }
     ...
     *
     * 2. sonara://watch?v=xxx, sonara://playlist?list=xxx, etc.
     *    → Convert to https://musicapp-lime.vercel.app/app/watch?v=xxx format
     *
     * 3. https://musicapp-lime.vercel.app/app/... or YouTube URLs
     *    → Pass through as-is
     */
    private fun parseToIntent(uri: String): GenericIntent {
        ...
        val actualUri = when {
            // sonara://open-app?url=<encoded_url>
            parsed.scheme == "sonara" && parsed.host == "open-app" -> {
                val urlParam = parsed.getQueryParameter("url")
                if (urlParam != null) {
                    Logger.d(TAG, "Extracted URL from open-app: $urlParam")
                    Uri.parse(urlParam)
                } else {
                    // sonara://open-app without params → just open the app, no navigation
                    Logger.d(TAG, "open-app without URL param, just opening app")
                    null
                }
            }

            // sonara://watch?v=xxx → https://musicapp-lime.vercel.app/app/watch?v=xxx
            // sonara://playlist?list=xxx → https://musicapp-lime.vercel.app/app/playlist?list=xxx
            // sonara://channel/UCxxx → https://musicapp-lime.vercel.app/app/channel/UCxxx
            // sonara://album?id=xxx → https://musicapp-lime.vercel.app/app/album?id=xxx
            parsed.scheme == "sonara" && parsed.host != null -> {
                val host = parsed.host!!
                val query = parsed.query?.let { "?$it" } ?: ""
                val pathSuffix = parsed.pathSegments.joinToString("/").let {
                    if (it.isNotEmpty()) "/$it" else ""
                }
                val convertedUrl = "https://musicapp-lime.vercel.app/app/$host$pathSuffix$query"
                Logger.d(TAG, "Converted sonara:// to: $convertedUrl")
