...
object WindowsProtocolRegistrar {
    ...
    private const val SCHEME = "sonara"
    ...
    fun register() {
        ...
        try {
            ...
            Logger.d(TAG, "Registering sonara:// protocol handler -> $exePath")

            // Main key with protocol description
            regAdd(REG_KEY, null, "URL:Sonara Music Protocol")
    ...
    private fun resolveExePath(): String? {
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "SonaraMusic.exe")
            if (exeFile.exists()) {
